/*
Source code from: https://github.com/wx257osn2/qoixx v0.1.4
By wx257osn2
MIT License
*/
...
//requires(sizeof(T) == 1) // Commented out to avoid: 'argument list for variable template "qoixx::detail::requires" is missing'
struct default_container_operator<std::vector<T, A>>{
  ...
  struct pusher{
    ...
    template<typename U>
    //requires std::unsigned_integral<U> && (sizeof(U) != 1) // Commented out to avoid compilation error
...
class qoi{
 ...
 public:
  ...
  struct desc{
    ...
    // constexpr bool operator==(const desc&)const noexcept = default; // Commented out to avoid 'cannot be defaulted' error
  };
  struct rgba_t{
    std::uint8_t r, g, b, a;
